...
function get_invoice_range($from, $to, $user=false, $loc=false) {
	global $SysPrefs;

	$ref = ($SysPrefs->print_invoice_no() == 1 ? "trans_no" : "reference");

	$sql = "SELECT trans.trans_no, trans.reference, u.user_id, au.stamp FROM ";

	if($loc)
		$sql .= TB_PREF."sales_orders o, ";

	$sql .= TB_PREF."debtor_trans trans";

	$sql .=	" LEFT JOIN ".TB_PREF."voided voided ON trans.type=voided.type AND trans.trans_no=voided.id";

	$sql .= " LEFT JOIN ".TB_PREF."audit_trail au ON trans.trans_no= au.trans_no AND trans.type = au.type";

	$sql .= " LEFT JOIN ".TB_PREF."users u ON u.id = au.user";

	$sql .= " WHERE trans.type=".ST_SALESINVOICE;

	if($loc)
		$sql .= " AND trans.order_ = o.order_no AND o.trans_type = ".ST_SALESORDER." AND o.from_stk_loc = ".db_escape($loc);
	
	$sql .= " AND ISNULL(voided.id)"
			." AND trans.tran_date >= '".date2sql($from)."' AND trans.tran_date <= '".date2sql($to)."'";			

	if ($user !== false)
		$sql .= " AND au.user = ".db_escape($user);

	$sql .= " GROUP BY trans.trans_no ORDER BY trans.tran_date, trans.$ref";
...
function print_invoices() {
	...
	$from = $_POST['PARAM_0'];
	$to = $_POST['PARAM_1'];
	$loc_filter = $_POST['PARAM_2'];
	$user = $_POST['PARAM_3'] == -1 ? false : $_POST['PARAM_3'];
	$email = $_POST['PARAM_4'];
	...
	//----------code-Desc-Qty--uom--tax--prc--Disc--Tot--//
	...
	$range = Array();
	$range = get_invoice_range($from, $to, $user, $loc_filter);
	...
	while($row = db_fetch($range)) {
		...
		$str_pad = strtoupper($_SESSION['wa_current_user']->prefs->pagesize) == 'LETTER' ? 33 : 27;
		$rep->NewLine();
		...
		$rep->TextWrap($rep->cols[4], $rep->row, $rep->cols[6], str_pad('', 10, '_'), 'L');
		$rep->NewLine();
		$rep->Text($rep->cols[4], _('Grand Total:'), $rep->cols[5]);
		$rep->Text($rep->cols[5], number_format2($Total, $dec), $rep->cols[6]-2, 0, 0, 'R');
		$rep->NewLine(1.5);
		$rep->Text($rep->cols[4], _('Paid Amount:'), $rep->cols[5]+4, 0, 0, 'L');
