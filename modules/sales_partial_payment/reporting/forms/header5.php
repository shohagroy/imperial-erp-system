...
if ($this->l['a_meta_dir'] == 'rtl') {// avoid line overwrite in rtl language
	...
	$this->LineTo($this->cols[$cols - 5]-2, $iline3 , $this->cols[$cols - 5]-2, $this->formData['line_bottom']);
...
//Terms & Condition
$this->Font('bold');
$this->TextWrap($this->cols[0], $iline7 + 8*$this->lineHeight, $this->pageWidth, _('Recd, in Good Condition'), 'left');
$this->Font();
$this->TextWrap($this->cols[0], $iline7 + 4*$this->lineHeight, $mcol, str_pad('', 22, '_'), 'L'); // Recd, in Good Condition
...
// Line headers
$this->headers = array(_('SL#'), _('Item Code'), _('Item Description'), '', _('Unit'), _('Quantity'));
...
// Comments
// $this->TextWrap($this->cols[3], $iline7 + 8*$this->lineHeight, $this->pageWidth, $this->params['comments'], 'left');
$this->row = $lower + (11*$this->lineHeight);
$this->TextWrapLines($this->cols[3]+50, ($page_width/2)-35, $this->params['comments'], 'center');
