...
include_once($path_to_root.'/modules/sales_partial_payment/includes/ui/po_ui.inc');
...
elseif (isset($_GET['NewInvoice'])) {
	...
	elseif(is_numeric($_GET['NewInvoice']) && $_GET['NewInvoice'] > 0 && isset($_GET['InvoiceNo'])) {
		create_new_po(ST_SUPPINVOICE, $_GET['NewInvoice']);
		clone_invoice(ST_SUPPINVOICE, $_GET['NewInvoice'], $_GET['InvoiceNo']);
	}
...
function check_data_auto($line_no, $item) {
		...
		$dec = get_qty_dec($item->stock_id);
		$min = 1 / pow(10, $dec);
		if (!check_num('qty'.$line_no, $min)) {
			$min = number_format2($min, $dec);
			display_error(_('The quantity of the order item must be numeric and not less than ').$min);
			set_focus('qty'.$line_no);
			return false;
		}
...
function handle_new_item_auto() {
	...
	if ($allow_update == true) { 
		...
		if ($allow_update == true) {
			...
			$price = get_purchase_price($_SESSION['PO']->supplier_id, $_POST['stock_id']);

			if ($allow_update)
				$_SESSION['PO']->add_to_order(count($_SESSION['PO']->line_items), $_POST['stock_id'], 1, get_post('stock_id_text'), $price, '', $_SESSION['PO']->trans_type == ST_PURCHORDER ? $_POST['req_del_date'] : '', 0, 0);
			else 
				 display_error(_('The selected item does not exist or it is a kit part and therefore cannot be purchased.'));
...
function can_commit() {
	...
	if (($_SESSION['PO']->trans_type == ST_SUPPRECEIVE || $_SESSION['PO']->trans_type == ST_SUPPINVOICE) 
		&& !is_date_in_fiscalyear($_POST['OrderDate']) && empty($_POST['InvoiceNo'])) {
	...
	if (!$_SESSION['PO']->order_no  && empty($_POST['InvoiceNo'])) {
	...
	if ($_SESSION['PO']->trans_type==ST_SUPPINVOICE && is_reference_already_there($_SESSION['PO']->supplier_id, get_post('supp_ref'), $_SESSION['PO']->order_no) && empty($_POST['InvoiceNo'])) {
	...
	if (can_commit()) {
		...
		if ($cart->order_no == 0) { // new po/grn/invoice
			...
			if (get_post('InvoiceNo') > 0) { // void old invoice and delete it's all payments, order...
				$old_inv = get_supp_trans($_POST['InvoiceNo'], ST_SUPPINVOICE);
				$old_audit_trail = get_audit_trail_last(ST_SUPPINVOICE, $_POST['InvoiceNo']);
				$alloc_result = get_allocatable_from_supp_transactions($old_inv['supplier_id'], $_POST['InvoiceNo'], ST_SUPPINVOICE);
				begin_transaction();
				void_transaction(ST_SUPPINVOICE, get_post('InvoiceNo'), Today(), 'Document Reenterd');

				foreach($alloc_result as $paymnt) {
					void_supp_payment($paymnt['type'], $paymnt['trans_no']);
				}

				$grns = get_po_grns($cart->old_order);
				foreach($grns as $grn) {
					void_grn($grn['id']);
				}

				delete_po($cart->old_order, ST_PURCHORDER);
			}

			$trans_no = add_direct_supp_trans($cart);

			if (get_post('InvoiceNo') > 0) {
				if($trans_no && is_array($old_audit_trail))
					update_audit_trial_user(ST_SUPPINVOICE, $trans_no, $old_audit_trail['user']);
				commit_transaction();
			}
			if ($trans_no) {
				unset($_SESSION['PO']);
...
foreach($_SESSION['PO']->line_items as $line=>$item) {
	

	if(input_changed('qty'.$line)) {
		$allow_update = check_data_auto($line, $item);
		if ($allow_update) {
			if ($item->qty_inv > input_num('qty'.$line) || $item->qty_received > input_num('qty'.$line)) {
			...
			$_SESSION['PO']->update_order_item($line, input_num('qty'.$line), $item->price, @$_POST['req_del_date'], $item->item_description);
			$Ajax->activate('_page_body');
		}
	}
	if(input_changed('price'.$line)) {
		$allow_update = check_data_auto($line, $item);
		if ($allow_update) {
			$_SESSION['PO']->update_order_item($line, $item->quantity, input_num('price'.$line), @$_POST['req_del_date'], $item->item_description);
			$Ajax->activate('_page_body');
		}
	}
}
...
custom_po_header($_SESSION['PO']);
echo '<br>';

display_custom_po_items($_SESSION['PO']);

start_table(TABLESTYLE2);


if ($_SESSION['PO']->trans_type == ST_SUPPINVOICE)
	user_cash_accounts_list_row(_('Payment:'), 'cash_account', null, false, _('Delayed'), true);
