...
function edit_link($row) {
	...
	if($row['type'] == ST_SALESINVOICE) {
		$ref = get_trans_ref(ST_SALESORDER, $row['order_']);
		
		// only direct invoices can be edited
		if($ref === 'auto' && $_SESSION['wa_current_user']->can_access('SA_SALESINVOICEEDIT'))
			return pager_link(_('Edit'), "/modules/sales_partial_payment/manage/sales_order_entry.php?NewInvoice=".$row['order_']."&InvoiceNo=".$row['trans_no'], ICON_EDIT);
	}
...
users_list_cells(_('By user:'), 'user_id', null, true);
...
// added for sms module
text_cells(_("Phone:"), 'phone', null, 15, 15);
...
//------------------------------------------------------------------------------------------------
$sql = get_customer_inquiry_sql(get_post('TransAfterDate'), get_post('TransToDate'), get_post('customer_id'), get_post('user_id'), get_post('filterType'), check_value('show_voided'), get_post('phone'));
...
$cols = array(
	...
	_("Phone") => array('name'=>'contact_phone'), 
	_("Branch") => array('ord'=>''), 
