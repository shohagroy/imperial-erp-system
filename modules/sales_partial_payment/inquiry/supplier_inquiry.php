...
function credit_link($row) {
	...
	if ($page_nested)
		return '';
	return $row['type'] == ST_SUPPINVOICE && $row['TotalAmount'] - $row['Allocated'] > 0 ? pager_link(_('Credit This'), '/modules/sales_partial_payment/manage/supplier_credit.php?New=1&invoice_no='.$row['trans_no'], ICON_CREDIT) : '';
}
...
function edit_link($row) {
	...
	if ($page_nested)
		return '';
	if($row['type'] == ST_SUPPINVOICE && $_SESSION['wa_current_user']->can_access('SA_PURCHASEINVOICEEDIT')) { // work only with direct invoice
		$supp_trans = new supp_trans(ST_SUPPINVOICE);
		read_supp_invoice($row['trans_no'], ST_SUPPINVOICE, $supp_trans);

		//get the first element of array and check if exist
		$first_elm = reset($supp_trans->grn_items);
		if ($first_elm !== false) { 
			$grn_batch = get_grn_batch_from_item($first_elm->id);
			$myrow = get_grn_batch($grn_batch);

			if($myrow['reference'] === 'auto') // only direct invoices can be edited
				return pager_link(_('Edit'), '/modules/sales_partial_payment/manage/po_entry_items.php?NewInvoice='.$myrow['purch_order_no'].'&InvoiceNo='.$row['trans_no'], ICON_EDIT);
